...
class TextGeneratorTests(StageTest):
    def generate(self):
        return [
            TestCase(stdin="test/corpus.txt"),
            TestCase(stdin="test/corpus.txt"),
            TestCase(stdin="test/corpus.txt")
        ]
    ...
    def check(self, reply, attach):
        ...
        if len(sentences) != 10:
            return CheckResult.wrong("You should output exactly 10 sentences! Every sentence should be in a new line.")

        for sentence in sentences:
            sent = sentence.split()
            if len(sent) < 5:
                return CheckResult.wrong("A pseudo-sentence should not be shorter than 5 tokens.")
            if len(set(sent)) == 1:
                return CheckResult.wrong("Invalid output. All words of a sentence are identical.")
            if not sent[0][0].isupper():
                return CheckResult.wrong("Every pseudo-sentence should start with a capitalized word.")
            if sent[0][-1] in punct:
                return CheckResult.wrong("The first token of a pseudo-sentence should not end with sentence-ending punctuation.")
            if sent[-1][-1] not in punct:
                return CheckResult.wrong("Every pseudo-sentence should end with a sentence-ending punctuation mark.")
